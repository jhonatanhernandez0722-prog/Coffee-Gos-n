...
def metrics(
    ...
    expense_rows = database.execute(select(day, func.sum(FinancialMovement.amount)).where(FinancialMovement.movement_type == "EXPENSE", FinancialMovement.created_at >= start, FinancialMovement.created_at < end).group_by(day).order_by(day)).all()
    ...
    method_rows = database.execute(select(FinancialMovement.payment_method, func.sum(FinancialMovement.amount)).where(FinancialMovement.movement_type == "EXPENSE", FinancialMovement.created_at >= start, FinancialMovement.created_at < end).group_by(FinancialMovement.payment_method)).all()
    category_rows = database.execute(select(ExpenseCategory.name, func.sum(FinancialMovement.amount)).join(FinancialMovement, FinancialMovement.expense_category_id == ExpenseCategory.id).where(FinancialMovement.movement_type == "EXPENSE", FinancialMovement.created_at >= start, FinancialMovement.created_at < end).group_by(ExpenseCategory.name).order_by(func.sum(FinancialMovement.amount).desc())).all()
    ...
    return MetricsResponse(income_by_day=daily_points(income_rows), expenses_by_day=daily_points(expense_rows), sales_by_day=daily_points(sales_rows), payment_methods=[MetricPoint(label=method or "Sin definir", value=total or 0) for method, total in method_rows], expense_categories=[MetricPoint(label=name, value=total or 0) for name, total in category_rows], total_income=total_income, total_expenses=total_expenses, total_sales=sum(count for _, count in sales_rows))