            ...
            {sellers.map((seller) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <span className="grid size-10 shrink-0 place-items-center bg-[var(--blue-light)] text-[var(--blue-main)]">
                    <UserCog size={18} />
                  </span>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs font-semibold ${seller.is_active ? "text-emerald-700" : "text-[var(--muted)]"}`}
                  >
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {editing ? (
              ...
              <Plus size={19} className="text-[var(--blue-main)]" />
            )}
          </div>
          <label className="mt-5 block text-sm font-semibold">
            ...
            <select
              value={role}
              onChange={(event) => setRole(event.target.value as "SELLER" | "VIEWER")}
          ...
          <fieldset className="mt-5">
            ...
            <div className="mt-2 grid gap-2">
              {sections.map((section) => (
                <label
                  key={section.key}
                  className="flex min-h-10 items-center gap-3 border border-[var(--line)] px-3 text-sm"
                >
                  <input
                    type="checkbox"
                  ...
                  {permissions.includes(section.key) && (
                    <Check size={15} className="ml-auto text-emerald-700" />
                  )}
                </label>
              ))}
            </div>
          ...
          <button
            disabled={saving}
            className="mt-6 min-h-11 w-full bg-[var(--blue-main)] px-4 text-sm font-semibold text-white disabled:opacity-60"
          >
            {saving
              ? "Guardando..."
              : editing
