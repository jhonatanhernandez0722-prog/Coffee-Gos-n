...
import { Check, Download, KeyRound, Mail, Pencil, Plus, UserCog, X } from "lucide-react";
import { AdminPage } from "@/components/admin-page";
import { downloadExcel } from "@/lib/excel";

type Seller = { id: number; full_name: string; email: string; is_active: boolean; permissions: string[] };
type PermissionKey = "dashboard" | "comanda" | "productos" | "clientes" | "creditos" | "movimientos" | "egresos" | "metricas";
...
const sections: { key: PermissionKey; label: string }[] = [
  { key: "dashboard", label: "Resumen" }, { key: "comanda", label: "Ventas" }, { key: "productos", label: "Productos" },
  { key: "clientes", label: "Clientes" }, { key: "creditos", label: "Créditos" }, { key: "movimientos", label: "Movimientos" },
  { key: "egresos", label: "Egresos" }, { key: "metricas", label: "Métricas" },
];
...
export default function SellersPage() {
  ...
  function exportSellers() { downloadExcel(sellers.map((seller) => ({ Nombre: seller.full_name, Correo: seller.email, Estado: seller.is_active ? "Activo" : "Inactivo", Permisos: seller.permissions.join(", ") })), "vendedores.xlsx", "Vendedores"); }
  ...
  return <AdminPage title="Vendedores" description="Crea accesos con PIN y decide qué apartados puede consultar cada persona del equipo.">
    ...
    <button type="button" onClick={exportSellers} className="mb-4 inline-flex min-h-10 items-center gap-2 border border-[var(--line)] bg-white px-4 text-sm font-semibold text-[var(--blue-main)]"><Download size={16} /> Descargar Excel</button>
