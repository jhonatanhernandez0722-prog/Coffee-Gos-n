...
type Receipt = { id: number; sale_number: string; total: number; payment_method: PaymentMethod; amount_received: number | null; change_amount: number; customer_name: string | null; created_at: string; items: { name: string; quantity: number; unit_price: number; line_total: number }[]; support_urls?: string[] };
...
export default function ComandaPage() {
  ...
  async function confirmSale() {
    ...
    try {
      ...
      if (paymentMethod === "CASH" && (received === null || !Number.isFinite(received) || received < total)) { setError("El dinero recibido debe ser igual o mayor que el total."); return; }
      const salePayload = { customer_id: customerId, buyer_name: buyerName.trim() || null, payment_method: paymentMethod, amount_received: received, assigned_seller_id: assignedSellerId, items: cart.map((item) => ({ product_id: item.id, quantity: item.quantity })) };
      ...
      setReceipt(saleResult); setCart([]); setBuyerName(""); setCustomerId(null); setAssignedSellerId(null); setPaymentMethod("CASH"); setAmountReceived(""); setSupportFiles([]);
  ...
  return <AdminPage title="Ventas" description="Registra ventas, selecciona el comprador y el método de pago.">
    ...
    {paymentMethod === "CASH" && <section className="mb-6 border border-[var(--line)] bg-white p-5"><label className="block text-sm font-semibold">Dinero recibido<input required min={total} step="1" inputMode="numeric" type="number" value={amountReceived} onChange={(event) => setAmountReceived(event.target.value.replace(/[^0-9]/g, ""))} className="mt-2 min-h-11 w-full border border-[var(--line)] px-3 font-normal" placeholder={String(total)} /><span className="mt-1 block text-xs font-normal text-[var(--muted)]">Vuelto: {amountReceived && Number(amountReceived) >= total ? money(Number(amountReceived) - total) : money(0)}</span></label></section>}
