...
export default function ComandaPage() {
  ...
  async function confirmSale() {
    ...
    try {
      const token = sessionStorage.getItem("coffee_gosen_access_token");
      const response = await fetch(`${apiUrl}/sales`, { method: "POST", headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ customer_id: customerId, buyer_name: buyerName.trim() || null, payment_method: paymentMethod, assigned_seller_id: assignedSellerId, items: cart.map((item) => ({ product_id: item.id, quantity: item.quantity })) }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail ?? "No fue posible confirmar la venta.");
      const saleResult = result as Receipt;
      setReceipt(saleResult); setCart([]); setBuyerName(""); setCustomerId(null); setAssignedSellerId(null); setPaymentMethod("CASH");
  ...
  async function uploadSupports() {
    if (!receipt || supportFiles.length === 0) return;
    setSaving(true); setError("");
    try {
      const token = sessionStorage.getItem("coffee_gosen_access_token");
      const formData = new FormData(); supportFiles.forEach((file) => formData.append("files", file));
      const response = await fetch(`${apiUrl}/sales/${receipt.id}/supports`, { method: "POST", headers: token ? { Authorization: `Bearer ${token}` } : undefined, body: formData });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail ?? "No fue posible guardar los soportes.");
      setReceipt((current) => current ? { ...current, support_urls: [...(current.support_urls ?? []), ...result.support_urls] } : current);
      setSupportFiles([]);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "No fue posible guardar los soportes."); } finally { setSaving(false); }
  }
  ...
  return <AdminPage title="Ventas" description="Registra ventas, selecciona el comprador y el método de pago.">
    {receipt && <div className="fixed inset-0 z-30 grid place-items-center bg-[var(--ink)]/30 p-4 print:hidden"><div className="w-full max-w-md border border-[var(--line)] bg-white p-5 shadow-xl"><h2 className="font-semibold">Soportes de la factura {receipt.sale_number}</h2><p className="mt-1 text-sm text-[var(--muted)]">Adjunta las fotos después de completar la venta.</p><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(event) => setSupportFiles(Array.from(event.target.files ?? []).slice(0, 5))} className="mt-4 block w-full text-sm" /><button type="button" disabled={saving || supportFiles.length === 0} onClick={uploadSupports} className="mt-4 min-h-10 bg-emerald-600 px-4 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Guardando..." : "Guardar soportes"}</button></div></div>}
